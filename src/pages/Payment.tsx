...
import { Check, Star, MessageCircle, ArrowRight, Loader2 } from "lucide-react";
import { initMercadoPago, Wallet } from '@mercadopago/sdk-react';

const Payment = () => {
    const [preferenceIdUnico, setPreferenceIdUnico] = useState<string | null>(null);
    const [preferenceIdCuotas, setPreferenceIdCuotas] = useState<string | null>(null);
    ...
    const handleBuy = async (productId: string) => {
        ...
        try {
            ...
            if (data.id) {
                if (productId === 'pago-unico') setPreferenceIdUnico(data.id);
                if (productId === 'cuotas') setPreferenceIdCuotas(data.id);
            } else {
                console.error("Error: No preference ID returned", data);
            }
        } catch (error) {
            ...
            <main className="container mx-auto px-4 py-16 md:py-24 max-w-5xl">
                ...
                >
                    ...
                    <p className="text-xl text-gray-light max-w-2xl mx-auto mb-4">
                        Elegí la opción de pago que prefieras. El proceso se completará de forma segura a través de Mercado Pago.
                    </p>
                ...
                <div className="mb-20">
                    ...
                    <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                        ...
                        <motion.div
                            ...
                            className="relative bg-white rounded-2xl p-8 text-charcoal border-2 border-coral shadow-xl transform hover:-translate-y-1 transition-transform duration-300 flex flex-col"
                        >
                            ...
                            <div className="mt-auto mb-6 flex flex-col justify-center min-h-[56px]">
                                {!preferenceIdUnico ? (
                                    <button
                                        onClick={() => handleBuy('pago-unico')}
                                        disabled={loadingProductId === 'pago-unico'}
                                        className="w-full bg-coral text-white font-bold text-center py-4 rounded-xl hover:bg-coral-dark transition-all shadow-lg hover:shadow-coral/30 flex items-center justify-center gap-2 group disabled:opacity-70 disabled:cursor-not-allowed"
                                    >
                                        {loadingProductId === 'pago-unico' ? (
                                            <>
                                    ...
                                    <div className="w-full animate-in fade-in zoom-in duration-300">
                                        <Wallet initialization={{ preferenceId: preferenceIdUnico, redirectMode: 'modal' }} customization={{ texts: { valueProp: 'security_safety' } }} />
                                    </div>
                                )}
                            </div>
                        ...
                        <motion.div
                            ...
                            className="relative bg-white/5 border border-white/10 rounded-2xl p-8 text-white hover:bg-white/10 transition-colors duration-300 flex flex-col"
                        >
                            ...
                            <div className="mt-auto mb-6 flex flex-col justify-center min-h-[56px]">
                                {!preferenceIdCuotas ? (
                                    <button
                                        onClick={() => handleBuy('cuotas')}
                                        disabled={loadingProductId === 'cuotas'}
                                        className="w-full bg-white text-charcoal font-bold text-center py-4 rounded-xl hover:bg-gray-100 transition-all shadow-lg flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                                    >
                                        {loadingProductId === 'cuotas' ? (
                                            <>
                                                <Loader2 size={18} className="animate-spin" />
                                                PROCESANDO...
                                            </>
                                    ...
                                    <div className="w-full bg-white rounded-xl overflow-hidden p-1 animate-in fade-in zoom-in duration-300">
                                        <Wallet initialization={{ preferenceId: preferenceIdCuotas, redirectMode: 'modal' }} customization={{ texts: { valueProp: 'security_safety' } }} />
                                    </div>
                                )}
                            </div>
